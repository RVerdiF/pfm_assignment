...
| `fct_revenue_attribution` | Marts (Implemented / Sample) | One row per valid conversion (`conversion_id`) | Filters `int_conversion_attribution` to valid conversions (`is_valid_conversion = true`: excludes denied orders, retains refunded orders). Exposes financial metrics (`commission_gbp` = `referral_bonus_gbp`), attributed marketing channel (`utm_source`, `utm_medium`, `utm_campaign`, `utm_content`), and attribution metadata (`match_status`, `match_method`). |
