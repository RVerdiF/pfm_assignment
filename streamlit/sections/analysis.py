...
def _render_overview(connection) -> None:
    ...
    if metrics["conversions"] and metrics["matched"] == 0:
        st.write(
            ...
            "and the unmatched card contains the entire cohort. Under exact "
            "click-identifier rules, no TrackNow click ID in this extract "
            "equals a PostHog click ID, so the pipeline correctly outputs zero "
            "matches instead of guessing a channel. That outcome is a "
            "property of this sample rather than a restatement of the "
            "reported 18% production gap. The breakdown below details where "
            "the non-matches fall."
        )
...
def _render_unmatched_diagnosis(connection) -> None:
    ...
    st.caption(
        ...
        "accounts for the reported 18% gap."
    )
    ...
    if total:
        ...
        st.write(
            ...
            "can also stem from redirect stripping or cookie resets, as "
            "detailed in the investigation plan under Methodology."
        )
